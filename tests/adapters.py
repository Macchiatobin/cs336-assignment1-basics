...
def run_scaled_dot_product_attention(
    ...
    """
    from cs336_basics.transformer.basic_modules import scaled_dot_product_attention
...
def run_multihead_self_attention(
    ...
    """
    from cs336_basics.transformer.basic_modules import MultiHeadSelfAttention
    mhsa_module=MultiHeadSelfAttention(
        d_model=d_model,
        num_heads=num_heads,
    )
    mhsa_module.load_param(
        W_Q=q_proj_weight,
        W_K=k_proj_weight,
        W_V=v_proj_weight,
        W_O=o_proj_weight,
    )
    return mhsa_module.forward(in_features)
...
def run_rope(
    ...
    """
    from cs336_basics.transformer.basic_modules import RotaryPositionalEmbedding
    rope_module = RotaryPositionalEmbedding(
        theta=theta,
        d_k=d_k,
        max_seq_len=max_seq_len,
    )
    return rope_module.forward(
        x=in_query_or_key,
        token_positions=token_positions,
    )
...
def run_transformer_block(
    ...
    transformer.load_param_for_test(weights)
...
def run_transformer_lm(
    ...
    lm.load_params_for_test(weights)
    return lm.forward(
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.transformer.basic_modules import softmax
    return softmax(
        x=in_features,
        dim=dim,
    )
